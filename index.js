...
const server = http.createServer(async (req, res) => {
  ...
  // Only accept POST to /sap/bc/http/sap/Z_API_ADDRESS_SPLIT
  // (same path as the BTP version so SF config needs no changes)
  if (req.method === 'POST' && req.url === '/sap/bc/http/sap/Z_API_ADDRESS_SPLIT') {
    ...
    req.on('end', async () => {
      try {
        if (!body) {
          res.writeHead(400); res.end(JSON.stringify({ error: 'Payload body is empty' })); return;
        }

        const parsed = JSON.parse(body);
        if (!parsed.address) {
          res.writeHead(400); res.end(JSON.stringify({ error: 'Please provide a valid JSON with an address field' })); return;
        }

        const results = await fetchNominatim(parsed.address);

        if (!results || results.length === 0) {
          res.writeHead(404); res.end(JSON.stringify({ error: 'Address not found' })); return;
        }
